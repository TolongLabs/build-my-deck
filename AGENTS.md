...
**build-my-deck** — a TolongLabs product that generates presentation-quality pitch decks from a prompt plus reference material, and lets the author edit the result element-by-element in a Canva-like browser editor.
...
- `~/CS/qwen/SahurHub/docs/demo/pitch-deck.html` — comic/halftone art direction, hand-authored inline SVG icon set, fixed 1920×1080 `<section class="slide">` stage.
- `~/CS/Layak/docs/demo-slides/layak-pitch-deck.html` — editorial art direction on a reusable `<deck-stage>` web component: shadow DOM, fixed design canvas scaled with `transform`, keyboard/tap nav, localStorage slide position, `@media print` pagination for PDF export, and a `data-om-validate="no_overflowing_text,no_overlapping_text,slide_sized_text"` contract.
- `/mnt/e/Projects/fyp-folder/FYP Part 2/final-report/slides/solarsim-slides.html` — the same `<deck-stage>` runtime under a different design system, confirming the runtime is already reusable across decks.

The common shape across all three: **slides are absolutely-positioned elements inside a fixed 1920×1080 coordinate space.** That is already a scene graph, and it is the bridge between the generator's output and the editor's document model.
...
- **Error handling:** Validate at system boundaries; do not wrap internal framework calls in try/catch. LLM and image-model responses are untrusted input — schema-validate them at the boundary.
...
- **Gate 2 (ship) mode:** `pr-auto` into `main` — the PM opens a PR and self-merges (`gh pr merge --squash --delete-branch`). A PR is opened **only for substantial changes / the end of an iteration**; small hotfixes or minor edits are committed + pushed straight to `main`. Merged PR branches are deleted so no residue is left. Agents never bypass this mode, never `--force`, and still require human authorization at Gate 2 before anything is pushed.
- **Workflow visibility:** `shared` — the pm-workflow artifacts are part of the repo like any other file, so every TolongLabs contributor running Claude Code or Codex gets the identical pipeline.
...
- **Codex delegation:** `executor, peer-consult` — each feature gates **independently**: **peer-consult** = a blind planning consult for high-stakes tasks (human-triggered per task, never automatic; planner and Codex get the same brief independently, neither sees the other's output, planner synthesizes and records where they disagreed); **executor** = Codex workers may implement PG tasks (same PG contract: no commits, no `docs/` writes, surgical changes, explicit file scope). **second-opinion is off** in this project — enable it via the upgrade flow if automatic cross-vendor QA review is wanted. Delegation applies **only when the main agent is Claude** — a Codex main agent uses the native `.codex/agents/` role subagents instead and ignores this mode. Every invocation is hardened: `codex --version` preflight, model + reasoning effort pinned from the profile matrix, stdin closed, event-silence liveness monitoring with a generous backstop timeout, and salvage of the last complete message on any kill — a hung or missing Codex never blocks the pipeline, and a slow-but-live one is never discarded; the PM degrades silently to Claude-only only when nothing is salvageable, and says so.
- **Design quality:** Impeccable is installed at project scope. Its PostToolUse hook gives implementers live design feedback on UI edits; QA runs `npx impeccable detect` on UI diffs as the deterministic audit (recorded in `docs/test.md`). `docs/PRODUCT.md` + `docs/DESIGN.md` are **product docs** — committed like `docs/prd.md`, always kept together in `docs/`. Never suppress a design finding without explicit human confirmation; waivers go through `/impeccable hooks ignore-*` only. **Note the double meaning here:** this product's *output* is design. Slide-quality rules (typography scale, contrast, whitespace, overflow) belong in the generator's own validation layer, not only in the repo's UI review.
...
- **Do not** `git push --force`, rewrite published history, or delete branches (other than the PR branch on squash-merge).
